...
struct CaptureFrameWriteResult {
  ...
  let bridgeTiming: BullRustBridgeTiming?
...
final class CaptureFrameEnqueueAggregator {
  ...
  private let queue = DispatchQueue(label: "com.bull.swift.capture-frame-enqueue", qos: .utility)
...
final class CaptureFrameWriteQueue: @unchecked Sendable {
  private let writeQueue = DispatchQueue(label: "com.bull.swift.capture-frame-writes", qos: .utility)
  private let stateLock = NSLock()
  private let rust = BullRustBridge()
      ...
      do {
        let report = try rust.request(
          ...
          args: [
            ...
            "parser_version": "bull-swift/live-notification",
