...
struct BullLocalDataExportResult {
  let url: URL
  let manifestURL: URL?
  let manifestError: String?
  let fileCount: Int
  let byteCount: UInt64
  let validation: BullLocalDataExportValidation
...
enum BullLocalDataExportError: LocalizedError, CustomStringConvertible {
...
struct BullLocalDataExportValidation {
  ...
  func withBundleJSONValidation(valid: Bool, error: String?) -> BullLocalDataExportValidation {
    BullLocalDataExportValidation(
  ...
  func withAdditionalIssue(_ issue: String) -> BullLocalDataExportValidation {
    BullLocalDataExportValidation(
      requiredOvernightSessionID: requiredOvernightSessionID,
...
struct BullOvernightExportMetrics {
...
enum BullLocalDataExporter {
  ...
  static func createBundle(requiredOvernightSessionID: String? = nil) throws -> BullLocalDataExportResult {
    ...
    let fileName = "bull-local-data-\(Int(now.timeIntervalSince1970))-\(exportID).bullbundle.json"
    ...
    do {
      ...
      try writeJSONObjectFields([
        "schema": "bull.local-data-export.v1",
        "export_id": exportID,
      ...
      if let validationError = bundleJSONStructureIssue(at: temporaryURL) {
        throw BullLocalDataExportError.invalidBundleJSON(validationError)
      }
      if fileManager.fileExists(atPath: outputURL.path) {
        throw BullLocalDataExportError.outputAlreadyExists(outputURL.path)
      }
  ...
  static func writeExportManifest(
    ...
    byteCount: UInt64,
    files: [[String: Any]],
    validation: BullLocalDataExportValidation
  ) throws -> URL {
    ...
    let object: [String: Any] = [
      "schema": "bull.local-data-export-manifest.v1",
