...
final class MoreDataStore: ObservableObject {
  ...
  @Published var storageNextAction = "Run Check after Bull has created the local database"
  ...
  @Published var existingBullRecordsStatus = "No local database checked"
  ...
  init(databasePath: String? = nil) {
    ...
    self.databasePath = databasePath ?? appDirectory.appendingPathComponent("bull.sqlite").path
  ...
  func routeStatus(ble: BullBLEClient, model: BullAppModel) -> MoreRouteStatus {
    MoreRouteStatus(
  ...
  func refreshBridgeStatus(model: BullAppModel) {
  ...
  func liveNotificationCaptureSummary(ble: BullBLEClient) -> String {
    if ble.connectionState == "ready" {
      return "Ready; notifications are mirrored through BullBLEClient.onNotification"
    }
    if ble.isScanning {
  ...
  func startCapture(ble: BullBLEClient) {
  ...
  func runStorageCheck() {
    guard databaseExists else {
      ...
      storageNextAction = "Start capture or run another bridge flow that creates bull.sqlite"
      existingBullRecordsStatus = "No Bull records"
      return
    }
    ...
    do {
      ...
      existingBullRecordsStatus = Self.recordCountSummary(value)
    } catch {
  ...
  func runAppleHealthDryRun() {
    healthSyncReports = ["Apple Health metric sync disabled; Bull metrics must come from WHOOP packets or local estimates."]
  }
  ...
  func runRawExport() {
    ...
    let zipPath = URL(fileURLWithPath: outputDirectory)
      .appendingPathComponent("bull-raw-export-\(Int(Date().timeIntervalSince1970)).zip")
      .path
    let args: [String: Any] = [
      "database_path": databasePath,
