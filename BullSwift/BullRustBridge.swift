...
enum BullRustBridgeError: Error {
  case encodingFailed
  case nullResponse
...
final class BullRustBridge {
  private var counter = 0
  private(set) var lastTiming: BullRustBridgeTiming?
  ...
  func requestValue(method: String, args: [String: Any] = [:]) throws -> Any {
    ...
    let payload: [String: Any] = [
      ...
      "request_id": "bull-swift-\(Date().timeIntervalSince1970)-\(counter)",
      "method": method,
      "args": args,
    ]
    ...
    guard let request = String(data: data, encoding: .utf8) else {
      throw BullRustBridgeError.encodingFailed
    }
    ...
    request.withCString { pointer in
      responsePointer = bull_bridge_handle_json(pointer)
    }
    let ffiRoundTripMicroseconds = Self.elapsedMicroseconds(since: ffiStarted)
    guard let responsePointer else {
      throw BullRustBridgeError.nullResponse
    }
    defer {
      bull_bridge_free_string(responsePointer)
    }
    ...
    guard
      ...
      throw BullRustBridgeError.malformedResponse
    }
    ...
    if !ok {
      ...
      throw BullRustBridgeError.methodFailed(message)
    }
