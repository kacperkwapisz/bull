...
extension BullBLEClient {
  ...
  func select(_ device: BullDiscoveredDevice) {
  ...
  func sendClientHello(reason: String, force: Bool) {
    ...
    guard
      ...
      !BullHello.clientHelloFrame.isEmpty
    else {
    ...
    emitCommandWrite(
      ...
      sequence: nil,
      payload: Data(),
      frame: BullHello.clientHelloFrame,
      peripheral: activePeripheral,
      characteristic: commandCharacteristic,
      writeType: writeType
    )
    ...
    record(
      source: "ble",
      title: "hello.sent",
      body: "reason=\(reason) \(commandCharacteristic.uuid.uuidString) \(writeTypeName(writeType)) \(BullHello.clientHelloFrameHex)"
    )
  }
