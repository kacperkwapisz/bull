...
struct MoreCaptureView: View {
  ...
  private func icon(for level: BullLogLevel) -> String {
    switch level {
    case .debug: "terminal"
  ...
  private func status(for level: BullLogLevel) -> MoreStatusKind {
    switch level {
  ...
  var body: some View {
    List {
      ...
      if !store.databaseExists {
        Section("Empty State") {
          MoreInfoRow(title: "No Database Yet", value: "The app has not created bull.sqlite at this path. Capture or bridge-backed flows can create it.", systemImage: "tray", status: .unavailable)
        }
      }
    }
    .bullListBackground()
  ...
  var body: some View {
    List {
      ...
      Section("Adapter") {
        ...
        MoreInfoRow(title: "Existing Bull Records", value: store.existingBullRecordsStatus, systemImage: "externaldrive", status: .pending)
