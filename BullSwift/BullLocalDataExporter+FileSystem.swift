...
extension BullLocalDataExporter {
  static func validateSQLiteDatabase(
    ...
    guard exists else {
      issues.append("missing bull.sqlite")
      return
    }
    if !included {
      issues.append("bull.sqlite was not included in export")
    }
    ...
    do {
      ...
      if !storageCheckPassed {
        ...
        if storageIssues.isEmpty {
          issues.append("bull.sqlite storage check failed")
        } else {
          issues.append("bull.sqlite storage check failed: \(storageIssues.joined(separator: "; "))")
        }
      }
    } catch {
      openable = false
      storageCheckPassed = false
      issues.append("bull.sqlite could not be opened: \(errorSummary(error))")
    }
  }
  ...
  static func defaultDatabasePath() -> String {
    ...
    return baseDirectory
      ...
      .appendingPathComponent("bull.sqlite")
      .path
  }
  ...
  static func includedBLELogURLs(
    ...
    var urls: [URL] = []
    if pathSet.contains("Application Support/BullSwift/bull-ble.log"),
       let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
      urls.append(
        appSupport
          .appendingPathComponent("BullSwift", isDirectory: true)
          .appendingPathComponent("bull-ble.log")
      )
    }
    if pathSet.contains("Documents/BullSwift/bull-ble-live.log") {
      urls.append(
        documentsDirectory
          .appendingPathComponent("BullSwift", isDirectory: true)
          .appendingPathComponent("bull-ble-live.log")
      )
    }
    return urls
  }
  ...
  static func errorSummary(_ error: Error) -> String {
    if case let BullRustBridgeError.methodFailed(message) = error {
      return message
    }
  ...
  static func exportRoots(fileManager: FileManager, documentsDirectory: URL) -> [(label: String, url: URL)] {
    ...
    if let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
      roots.append(("Application Support/BullSwift", appSupport.appendingPathComponent("BullSwift", isDirectory: true)))
    }
    roots.append(("Documents/BullSwift", documentsDirectory.appendingPathComponent("BullSwift", isDirectory: true)))
  ...
  static func shouldIncludeFile(
    ...
    switch label {
    case "Application Support/BullSwift":
      return relativePath == "bull.sqlite"
        || relativePath == "bull.sqlite-wal"
        || relativePath == "bull.sqlite-shm"
        || relativePath == "bull-ble.log"
        || relativePath.hasPrefix("bull-ble.")
    case "Documents/BullSwift":
      return relativePath == "bull-ble-live.log"
