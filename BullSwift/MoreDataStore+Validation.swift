...
extension MoreDataStore {
  ...
  nonisolated static func rawValidationRunbookMarkdown(
    ...
    throw NSError(
      domain: "BullRawValidationRunbook",
      code: 1,
  ...
  func saveLocalDataBundle() {
    ...
    DispatchQueue.global(qos: .userInitiated).async {
      do {
        let result = try BullLocalDataExporter.createBundle()
  ...
  func runFrameParseProbe() {
    do {
      let value = try bridge.request(
        ...
        args: [
          "device_type": "BULL",
          "frame_hex": BullHello.clientHelloFrameHex,
        ]
      )
  ...
  static func previewDefault() -> MoreDataStore {
    let store = MoreDataStore(databasePath: "/tmp/bull-preview.sqlite")
  ...
  static func previewConnected() -> MoreDataStore {
    ...
    store.liveCaptureStatus = "Ready; notifications are mirrored through BullBLEClient.onNotification"
