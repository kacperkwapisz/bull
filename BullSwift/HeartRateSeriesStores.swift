...
final class HeartRateSeriesStore {
  ...
  static let didUpdateNotification = Notification.Name("BullHeartRateSeriesStoreDidUpdate")
  ...
  private let writeQueue = DispatchQueue(label: "com.bull.swift.heart-rate-series", qos: .utility)
  ...
  private static func persist(payload: HeartRateSeriesFile, to url: URL) {
    do {
      ...
      NSLog("BullSwift heart-rate sample persist failed: \(String(describing: error))")
    }
  }
...
final class HRVSeriesStore {
  ...
  static let didUpdateNotification = Notification.Name("BullHRVSeriesStoreDidUpdate")
  ...
  private let url: URL
  private let stateLock = NSLock()
  private let writeQueue = DispatchQueue(label: "com.bull.swift.hrv-series", qos: .utility)
  ...
  private static func loadPersistedLiveSample() -> HRVSamplePoint? {
    ...
    guard defaults.object(forKey: "bull.swift.liveHRVRMSSD") != nil else {
      return nil
    }
    let rmssd = defaults.double(forKey: "bull.swift.liveHRVRMSSD")
    let rrIntervalCount = defaults.integer(forKey: "bull.swift.liveHRVRRIntervalCount")
    let source = defaults.string(forKey: "bull.swift.liveHRVSource") ?? "ble.hr.standard.average.migrated"
    let capturedAt = defaults.object(forKey: "bull.swift.liveHRVUpdatedAt") as? Date ?? Date()
  ...
  private static func persist(payload: HRVSeriesFile, to url: URL) {
    do {
      ...
      NSLog("BullSwift HRV sample persist failed: \(String(describing: error))")
    }
  }
...
final class HeartRateSamplePipeline {
  ...
  private let queue = DispatchQueue(label: "com.bull.swift.heart-rate-sample-pipeline", qos: .utility)
