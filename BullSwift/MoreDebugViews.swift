...
struct MoreDebugView: View {
  ...
  var body: some View {
    List {
      ...
      Section("Research BT Commands") {
        ...
        MoreInfoRow(
          title: "Remote Calls",
          value: "bullswift://debug-command/<id>?payload=<hex>",
          systemImage: "link",
          status: .pending
        )
  ...
  private func debugCommandActionStatus(_ command: BullDebugCommandDefinition) -> MoreStatusKind {
    if model.ble.connectionState != "ready" {
      return .blocked
    }
  ...
  private func debugCommandDetail(_ command: BullDebugCommandDefinition) -> String {
    "id \(command.id) | cmd \(command.commandNumber) | \(command.payloadHint) | \(command.risk)"
  }

  private func debugCommandResponseDetail(_ response: BullDebugCommandResponse) -> String {
  ...
  private func debugCommandIcon(_ command: BullDebugCommandDefinition) -> String {
