...
enum CoachTipFactory {
  static func homeTip(healthStore: HealthDataStore, appModel: BullAppModel) -> CoachInlineTip {
    ...
    return CoachInlineTip(
      ...
      prompt: """
      Give me today's coaching priority from my local Bull context. Use readiness, sleep, recovery, strain, stress, live heart rate, and missing-data gaps. Cite the local tool outputs and keep it to one concrete next action.
  ...
  static func metricTip(
    ...
    default:
      ...
      return CoachInlineTip(
        ...
        prompt: "Explain my \(route.title.lowercased()) page using the local Bull context. Cite the tool outputs and call out stale or missing data.",
        systemImage: "sparkles",
        tint: snapshot.tint
      )
    }
  ...
  static func sleepTip(healthStore: HealthDataStore, ble: BullBLEClient) -> CoachInlineTip {
    ...
    return CoachInlineTip(
      ...
      prompt: """
      Explain my sleep page and give one practical next action. Use only local Bull context and call out missing data and provenance.
  ...
  private static func strainTip(healthStore: HealthDataStore, appModel: BullAppModel) -> CoachInlineTip {
  ...
  private static func stressTip(healthStore: HealthDataStore, appModel: BullAppModel) -> CoachInlineTip {
