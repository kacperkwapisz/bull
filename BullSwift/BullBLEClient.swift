...
final class BullBLEClient: NSObject, ObservableObject {
  @Published var bluetoothState = "not requested"
  @Published var connectionState = "disconnected"
  @Published var isScanning = false
  @Published var discoveredDevices: [BullDiscoveredDevice] = []
  ...
  @Published var debugCommandResponses: [BullDebugCommandResponse] = []
  @Published var debugCommandSnapshotPath = "No debug command snapshot"

  var onNotification: ((BullNotificationEvent) -> Void)?
  var onRawNotification: ((BullNotificationEvent) -> Void)?
  var onRawNotificationWithContext: ((BullNotificationEvent, BullBLENotificationContext) -> Void)?
  var onCommandWrite: ((BullCommandWriteEvent) -> Void)?
  ...
  var onHistoricalSyncProgress: ((BullHistoricalSyncProgress) -> Void)?
  var onHistoricalRangeTelemetry: ((BullHistoricalRangeTelemetry) -> Void)?
  var onMessage: ((BullMessage) -> Void)?

  let logger = Logger(subsystem: "com.bull.swift", category: "ble")
  let coreBluetoothQueue = DispatchQueue(label: "com.bull.swift.corebluetooth", qos: .utility)
  let realtimeVitalsQueue = DispatchQueue(label: "com.bull.swift.realtime-vitals", qos: .userInitiated)
  let diagnosticLogQueue = DispatchQueue(label: "com.bull.swift.diagnostic-log", qos: .utility)
  let bleUIStateAggregator = BLEUIStateAggregator(publishInterval: BullBLEClient.bleUIStatePublishInterval)
  let messageStore = BullMessageStore(
    maximumMessages: BullBLEClient.maximumDisplayedMessages,
    flushInterval: BullBLEClient.displayedMessageFlushInterval
  )
  ...
  let autoStartPhysiologyCaptureOnReady: Bool = {
    ...
    return processInfo.arguments.contains("--bull-start-physiology-capture")
      || processInfo.environment["BULL_START_PHYSIOLOGY_CAPTURE"] == "1"
  }()
  ...
  let diagnosticLoggingEnabled: Bool = {
    let processInfo = ProcessInfo.processInfo
    if processInfo.arguments.contains("--bull-disable-diagnostics")
      || processInfo.environment["BULL_DISABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["BULL_DIAGNOSTIC_LOGGING"] == "0" {
      return false
    }
    return processInfo.arguments.contains("--bull-enable-diagnostics")
      || processInfo.environment["BULL_ENABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["BULL_DIAGNOSTIC_LOGGING"] == "1"
  }()
  let prioritizeLiveCaptureOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--bull-start-physiology-capture")
      || processInfo.arguments.contains("--bull-start-health-packet-capture")
      || processInfo.arguments.contains("--bull-start-temperature-packet-capture")
      || processInfo.environment["BULL_START_PHYSIOLOGY_CAPTURE"] == "1"
      || processInfo.environment["BULL_START_HEALTH_PACKET_CAPTURE"] == "1"
      || processInfo.environment["BULL_START_TEMPERATURE_PACKET_CAPTURE"] == "1"
  }()
  let autoSendDebugSkinTemperatureCommand: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--bull-send-debug-skin-temp-command")
      || processInfo.environment["BULL_SEND_DEBUG_SKIN_TEMP_COMMAND"] == "1"
  }()
  let forceDebugMenuWrite: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--bull-force-debug-menu-write")
      || processInfo.environment["BULL_FORCE_DEBUG_MENU_WRITE"] == "1"
  }()
  let consoleCaptureStatusEnabled: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--bull-console-capture-status")
      || processInfo.environment["BULL_CONSOLE_CAPTURE_STATUS"] == "1"
  }()
  let debugSkinTemperatureCommandPayload: Data = {
    let processInfo = ProcessInfo.processInfo
    if let hex = processInfo.environment["BULL_DEBUG_MENU_COMMAND_HEX"],
       let data = Data(hexString: hex),
       !data.isEmpty {
      return data
    }
    if let text = processInfo.environment["BULL_DEBUG_MENU_COMMAND"],
  ...
  let diagnosticLogURL: URL? = {
    let processInfo = ProcessInfo.processInfo
    let loggingEnabled = processInfo.arguments.contains("--bull-enable-diagnostics")
      || processInfo.environment["BULL_ENABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["BULL_DIAGNOSTIC_LOGGING"] == "1"
    let loggingDisabled = processInfo.arguments.contains("--bull-disable-diagnostics")
      || processInfo.environment["BULL_DISABLE_DIAGNOSTICS"] == "1"
      || processInfo.environment["BULL_DIAGNOSTIC_LOGGING"] == "0"
    ...
    guard let directory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
      BullBLEClient.recordDiagnosticLogSetupWarning("bull-ble.log setup failed: Application Support directory unavailable")
      return nil
    }
    let bullDirectory = directory.appendingPathComponent("BullSwift", isDirectory: true)
    let url = bullDirectory.appendingPathComponent("bull-ble.log")
    do {
      try BullBLEClient.prepareDiagnosticLogDirectory(bullDirectory)
      if FileManager.default.fileExists(atPath: url.path) {
        try FileManager.default.removeItem(at: url)
      }
    } catch {
      BullBLEClient.recordDiagnosticLogSetupWarning("bull-ble.log setup failed: \(String(describing: error))")
      return nil
    }
    return url
  }()
  let diagnosticLogMirrorURL: URL? = {
    let processInfo = ProcessInfo.processInfo
    let mirrorEnabled = processInfo.arguments.contains("--bull-afc-diagnostic-mirror")
      || processInfo.environment["BULL_AFC_DIAGNOSTIC_MIRROR"] == "1"
    guard mirrorEnabled else {
      return nil
    }
    guard let directory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
      BullBLEClient.recordDiagnosticLogSetupWarning("bull-ble-live.log mirror setup failed: Documents directory unavailable")
      return nil
    }
    let bullDirectory = directory.appendingPathComponent("BullSwift", isDirectory: true)
    let url = bullDirectory.appendingPathComponent("bull-ble-live.log")
    do {
      try BullBLEClient.prepareDiagnosticLogFile(at: url, directory: bullDirectory)
    } catch {
      BullBLEClient.recordDiagnosticLogSetupWarning("bull-ble-live.log mirror setup failed: \(String(describing: error))")
      return nil
    }
    return url
  }()
  let overnightSideChannelLogURL: URL? = {
    guard let directory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
      BullBLEClient.recordDiagnosticLogSetupWarning("bull-ble-live.log setup failed: Documents directory unavailable")
      return nil
    }
    let bullDirectory = directory.appendingPathComponent("BullSwift", isDirectory: true)
    let url = bullDirectory.appendingPathComponent("bull-ble-live.log")
    do {
      ...
      BullBLEClient.recordDiagnosticLogSetupWarning("bull-ble-live.log setup failed: \(String(describing: error))")
      return nil
    }
  }()
  ...
  enum DefaultsKey {
    static let rememberedDeviceID = "bull.swift.rememberedDeviceID"
    static let rememberedDeviceName = "bull.swift.rememberedDeviceName"
    static let rememberedDeviceValidated = "bull.swift.rememberedDeviceValidatedWhoop"
    ...
    static let restingHeartRateEstimateSource = "bull.swift.restingHeartRateEstimateSource"
    static let liveHRVRMSSD = "bull.swift.liveHRVRMSSD"
    static let liveHRVRRIntervalCount = "bull.swift.liveHRVRRIntervalCount"
    static let liveHRVRMSSDSampleCount = "bull.swift.liveHRVRMSSDSampleCount"
    static let liveHRVUpdatedAt = "bull.swift.liveHRVUpdatedAt"
    static let liveHRVSource = "bull.swift.liveHRVSource"
    static let debugHistoricalRangeStatus = "bull.swift.debug.historicalRangeStatus"
  }

  static let restorationIdentifier = "com.bull.swift.central"
  ...
  enum ClockCommandKind {
    ...
    var payload: [UInt8] {
      ...
      case .get:
        return []
      case .set(let date):
        let timestamp = BullBLEClient.clockTimestampParts(for: date)
        var bytes: [UInt8] = []
        BullBLEClient.appendUInt32LE(timestamp.seconds, to: &bytes)
        BullBLEClient.appendUInt32LE(timestamp.subseconds, to: &bytes)
        return bytes
      }
    }
  ...
  struct AlarmHapticsPattern {
    ...
    var payloadBytes: [UInt8] {
      ...
      BullBLEClient.appendUInt16LE(loopControl, to: &bytes)
  ...
  enum AlarmCommandKind {
    ...
    var payload: [UInt8] {
      ...
      case .set(let alarmID, let date, let pattern):
        ...
        let timestamp = BullBLEClient.alarmTimestampParts(for: date)
        BullBLEClient.appendUInt32LE(timestamp.seconds, to: &bytes)
        BullBLEClient.appendUInt16LE(timestamp.subseconds, to: &bytes)
  ...
  static let debugResearchCommandDefinitions: [BullDebugCommandDefinition] = [
    BullDebugCommandDefinition(
  ...
  var debugResearchCommands: [BullDebugCommandDefinition] {
