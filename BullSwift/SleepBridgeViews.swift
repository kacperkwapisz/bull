  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(spacing: 8) {
        ...
        HealthInfoRow(row: HealthSummaryRow("Bull sleep score", value: store.sleepFeatureScoreSummary(), source: store.packetScoreSource("metrics.sleep_score_from_features"), systemImage: "bed.double"))
      }
      HStack(spacing: 10) {
        Button {
...
struct SleepAlarmBridgeSection: View {
  ...
  private var alarmSource: HealthDataSource {
    ...
    if ble.canWriteAlarm {
      return .live("BullBLEClient alarm write")
    }
