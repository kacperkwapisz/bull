...
@MainActor
final class BullAppModel: ObservableObject {
  ...
  @Published var overnightGuardWarning = "Keep the official WHOOP app closed until Bull final sync/export finishes."
  ...
  let notificationIngestQueue = DispatchQueue(label: "com.bull.swift.notification-ingest", qos: .utility)
  let notificationIngestStateLock = NSLock()
  let notificationParseQueue = DispatchQueue(label: "com.bull.swift.notification-parse", qos: .utility)
  ...
  let heartRateSamplePipeline = HeartRateSamplePipeline(
    timelinePublishInterval: BullAppModel.heartRateHourlyRangePublishInterval
  )
  let packetUIStateAggregator = PacketUIStateAggregator(
    publishInterval: BullAppModel.packetUIStatePublishInterval,
    maximumPendingDeviceSignalPoints: BullAppModel.maxRecentDeviceSignalPoints
  )
  ...
  let healthPacketCaptureFamilyAggregator = HealthPacketCaptureFamilyAggregator(
    publishInterval: BullAppModel.healthPacketCaptureUIUpdateInterval
  )
  let captureFrameWriteQueue = CaptureFrameWriteQueue(
    databasePath: HealthDataStore.defaultDatabasePath(),
    maxQueuedRows: BullAppModel.captureFrameWriteQueueMaxRows,
    maxBatchRows: BullAppModel.captureFrameWriteBatchMaxRows,
    coalesceDelay: BullAppModel.captureFrameWriteCoalesceDelay
  )
  ...
  let autoStartHealthPacketCaptureOnReady: Bool = {
    ...
    return processInfo.arguments.contains("--bull-start-health-packet-capture")
      || processInfo.environment["BULL_START_HEALTH_PACKET_CAPTURE"] == "1"
  }()
  let autoStartTemperaturePacketCaptureOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--bull-start-temperature-packet-capture")
      || processInfo.environment["BULL_START_TEMPERATURE_PACKET_CAPTURE"] == "1"
  }()
  let autoStartPhysiologyPacketCaptureOnReady: Bool = {
    let processInfo = ProcessInfo.processInfo
    return processInfo.arguments.contains("--bull-start-physiology-packet-capture")
      || processInfo.environment["BULL_START_PHYSIOLOGY_PACKET_CAPTURE"] == "1"
  }()
  ...
  let autoSyncHistoryDuringPhysiologyCapture: Bool = {
    ...
    return processInfo.arguments.contains("--bull-sync-history-during-physiology-capture")
      || processInfo.environment["BULL_SYNC_HISTORY_DURING_PHYSIOLOGY_CAPTURE"] == "1"
  }()
  let captureStatusSnapshotURL: URL? = {
    let processInfo = ProcessInfo.processInfo
    let enabled = processInfo.arguments.contains("--bull-afc-capture-status")
      || processInfo.environment["BULL_AFC_CAPTURE_STATUS"] == "1"
    guard enabled else {
      return nil
    }
    ...
    try? FileManager.default.createDirectory(at: bullDirectory, withIntermediateDirectories: true)
    return bullDirectory.appendingPathComponent("capture-status.txt")
  }()
  ...
  init(startBLE: Bool = true) {
    ble = BullBLEClient(startCentral: startBLE)
