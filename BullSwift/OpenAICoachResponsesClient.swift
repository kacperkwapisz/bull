...
enum OpenAICoachRequestFactory {
  ...
  You are Bull Coach inside a user-owned WHOOP companion app. Use the available Bull tools before making claims about health, activity, capture coverage, or device state. Cite tool names inline for metric claims, keep coaching practical, and say when data is missing or stale. Do not diagnose, prescribe, or infer medical conditions. Prefer one concrete next action when the local data is incomplete.
  """
  ...
  private static let tools: [[String: Any]] = [
    [
      "type": "function",
      "name": "load_stats",
      "description": "Load the current local Bull metric snapshot, readiness status, score summaries, live heart-rate summary, and provenance.",
...
struct OpenAIResponsesClient {
  ...
  func stream(
    ...
    request.setValue("bull-swift", forHTTPHeaderField: "originator")
