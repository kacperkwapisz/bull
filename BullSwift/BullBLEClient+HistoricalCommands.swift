...
extension BullBLEClient {
  func beginHistoricalSync(
    ...
    guard supportsV5HistoricalSync else {
      ...
      failHistoricalSync("Historical sync currently supports the Bull V5 fd4b command characteristic. Active command characteristic: \(characteristic).")
      return
    }
  ...
  func debugCommandPayload(
    for definition: BullDebugCommandDefinition,
    payloadHex: String?
  ) -> [UInt8]? {
