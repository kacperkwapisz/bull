...
extension BullLocalDataExporter {
  static func validate(
    ...
    let exportSelfIncluded = exportedRelativePaths.contains { path in
      path == "Documents/BullSwift/Exports" || path.hasPrefix("Documents/BullSwift/Exports/")
    }
    ...
    let sqliteDatabaseIncluded = pathSet.contains("Application Support/BullSwift/bull.sqlite")
    ...
    let bleLiveLogRelativePath = "Documents/BullSwift/bull-ble-live.log"
    ...
    var metrics = BullOvernightExportMetrics()
    var issues: [String] = []
    ...
    if let requiredOvernightSessionID {
      let base = "Documents/BullSwift/OvernightGuard/\(requiredOvernightSessionID)/"
    ...
    if exportSelfIncluded {
      issues.append("export contains files from Documents/BullSwift/Exports")
    }
    ...
    return BullLocalDataExportValidation(
