...
extension BullAppModel {
  ...
  func updateOvernightGuardWarning() {
    var warnings = ["Keep the official WHOOP app closed until Bull final sync/export finishes."]
  ...
  func refreshOvernightWatchdogState(reason: String) {
    ...
    if snapshot.notificationCount == 0, elapsed >= Self.overnightGuardRawStaleWarningInterval {
      ...
      let warning = "No raw BLE notifications after \(minutes)m; confirm WHOOP is connected and Bull is foregrounded if possible."
