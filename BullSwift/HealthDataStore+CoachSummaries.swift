...
extension HealthDataStore {
  ...
  func hrvFeatureProvenanceSummary() -> String {
    ...
    let algorithm = Self.map(report, "score_result")?["algorithm_id"] as? String ?? "bull.hrv.v0"
  ...
  func sleepV1ModelStatusSummary() -> String {
    ...
    let trusted = Self.intValue(status["trusted_bull_sleep_nights"]) ?? 0
    return "\(state.capitalized) | \(nights) imported nights | \(trusted) trusted bull nights"
  }
