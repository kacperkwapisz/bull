...
struct OnboardingProfileStep: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 18) {
      Text("These basics help Bull calculate your local metrics.")
        .font(.body)
        .foregroundStyle(.secondary)
...
struct OnboardingConnectStep: View {
  ...
  private var connectBody: String {
    if canVerify {
      return "Finish setup to start using Bull with this strap."
    }
    if connected {
      return "Keep the strap close while Bull confirms it can read data."
    }
    if hasDiscoveredStraps {
      return "Select the strap you want to use with Bull."
    }
    if searching {
...
struct OnboardingDiscoveredStrapRow: View {
  let device: BullDiscoveredDevice
  let selected: Bool
  let action: () -> Void
