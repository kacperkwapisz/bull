...
extension BullBLEClient {
  ...
  func failHistoricalSync(_ message: String) {
    ...
    let failure = BullSyncFailure(title: "Sync Failed", message: message, occurredAt: Date())
  ...
  func publishSyncToast(
    ...
    syncToast = BullSyncToast(phase: phase, title: titleOverride ?? title, detail: detail)
