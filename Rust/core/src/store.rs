...
mod tests {
    ...
    fn read_write_open_applies_local_storage_pragmas() {
        ...
        let path = dir.path().join("goose.sqlite");
    ...
    fn open_existing_current_reuses_current_local_database() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("goose.sqlite");
        GooseStore::open(&path).unwrap();

        let store = GooseStore::open_existing_current(&path).unwrap();
    ...
    fn open_existing_current_rejects_old_schema() {
        ...
        let error = GooseStore::open_existing_current(&path).unwrap_err();
    ...
    fn index_exists(store: &GooseStore, name: &str) -> bool {
        let count: i64 = store
            .conn
            .query_row(
