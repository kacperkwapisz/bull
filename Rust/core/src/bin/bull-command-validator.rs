...
use bull_core::{
    commands::{
        ...
        command_evidence_with_local_frame_matches, load_command_evidence,
...
fn run() -> bull_core::BullResult<()> {
    ...
    let evidence = match path_value(&args, "--evidence")? {
    ...
    let exit_pass = if flag(&args, "--capture-plan") {
        let requested_commands = value(&args, "--commands")?
            ...
            .unwrap_or_default();
        let plan = command_capture_plan_from_results(&report.commands, &requested_commands);
        write_json_report(&plan, output.as_deref())?;
        report.evidence_valid && plan.pass
    } else {
