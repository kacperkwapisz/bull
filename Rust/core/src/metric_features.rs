...
pub fn run_hrv_feature_report(
    ...
    // Process R20 frames in 5-minute chunks (~300 frames × 25 samples = 7500
    // samples at 25 Hz). Each chunk produces one HrvFeature with its RR
    // intervals. Chunking keeps the bandpass filter and peak detector stable
    // (full-window processing over hours produces noisy results).
    {
        let r20_chunk_target = 300usize; // ~5 min at 1 frame/sec
        let mut chunk_buf: Vec<i32> = Vec::new();
        let mut chunk_frame_count = 0usize;
        let mut chunk_frame_id = String::new();
        let mut chunk_evidence_id = String::new();
        ...
        for row in decoded_rows {
            ...
            if let Some(ParsedPayload::DataPacket {
                ...
                if chunk_frame_count == 0 {
                    ...
                    chunk_captured_at = row.captured_at.clone();
                }
                chunk_buf.extend_from_slice(ppg);
                chunk_frame_count += 1;
                total_r20 += 1;
